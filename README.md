...
<p align="center">
  ...
  <img alt="الإصدار" src="https://img.shields.io/badge/version-2.3.1-7FB069">
</p>
...
قسم **«Assets»** ونزّل **`Satr.Setup.2.4.0.exe`** — ثم شغّله (واجهة التثبيت عربية بالكامل).
إن لم يكن Claude Code مثبّتاً، يرشدك معالج أول التشغيل لتثبيته وتسجيل الدخول.

> [!WARNING]
> **لا تنزّل «Source code (zip/tar.gz)»** — هذا أرشيف اسمه `Satr-2.4.0.zip` (بشرطة)
> يحوي **شفرة المصدر** (مجلدات `electron` و`src` وملف `package.json`…) **وليس فيه
> التطبيق**. ملف التطبيق اسمه `Satr.Setup.2.4.0` (بنقاط وكلمة **Setup**) تحت **Assets**.
...
1. **نزّل `Satr.Setup.2.4.0.zip`** (تحت Assets، بكلمة Setup — لا «Source code») بدل الـ
...
3. **تنزيل مباشر يتجاوز المتصفح كلياً** — الصق في PowerShell:
   ```powershell
   irm "https://github.com/m4u05999/Satr/releases/latest/download/Satr.Setup.2.4.0.exe" -OutFile "$HOME\Downloads\Satr.Setup.exe"
   ```

**تحقّق من سلامة الملف (اختياري)** — يجب أن يطابق بصمة SHA-256 لإصدار v2.4.0:
```powershell
(Get-FileHash "$HOME\Downloads\Satr.Setup.2.4.0.exe" -Algorithm SHA256).Hash
# C8EAD69F16236DD671F4A1001FA1868C8525E06F87439A606A3EF5938D0714FC
```

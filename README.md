...
<p align="center">
  ...
  <img alt="الإصدار" src="https://img.shields.io/badge/version-2.16.21-7FB069">
</p>
...
### ⭐ الطريق الموصى به: متجر مايكروسوفت

<div align="center">

[**⬇ ثبّت «سطر» من Microsoft Store**](https://apps.microsoft.com/detail/9N7F5RKQJ9WF)

</div>

أو من PowerShell مباشرةً:

```powershell
winget install --id 9N7F5RKQJ9WF --source msstore
```

**لماذا هذا الطريق أفضل**: مايكروسوفت **توقّع الحزمة بنفسها**، فلا يمنعك المتصفح ولا
يظهر تحذير SmartScreen — وهما الحاجزان الموصوفان في القسمين التاليين. والتحديثات تصل
عبر المتجر تلقائياً.

> [!NOTE]
> نسخة المتجر تعمل داخل حاوية Windows، والتطبيق فيها **يقرأ ويكتب في مجلدات مشاريعك
> عادةً** وتعمل الطرفية المدمجة كاملةً (مقيس قبل النشر). الفارق الوحيد: زرّ «تحقّق من
> التحديثات» مخفيّ لأن المتجر يتولّى ذلك، وتظهر مكانه شارة **«نسخة المتجر»**.

### الطريق الثاني: المثبّت من GitHub

في [صفحة الإصدارات (Releases)](https://github.com/m4u05999/Satr/releases/latest)، انزل إلى
قسم **«Assets»** ونزّل **`Satr-Setup-2.16.21.exe`** — ثم شغّله (واجهة التثبيت عربية بالكامل).
إن لم يكن Claude Code مثبّتاً، يرشدك معالج أول التشغيل لتثبيته وتسجيل الدخول.

> [!TIP]
> القسمان التاليان (منع المتصفح وتحذير SmartScreen) يخصّان **هذا الطريق وحده** —
> ولا يظهر أيٌّ منهما إن ثبّتّ من المتجر أعلاه.

> [!WARNING]
> **لا تنزّل «Source code (zip/tar.gz)»** — هذا أرشيف اسمه `Satr-2.16.21.zip` (بشرطة)
> يحوي **شفرة المصدر** (مجلدات `electron` و`src` وملف `package.json`…) **وليس فيه
> التطبيق**. ملف التطبيق فيه كلمة **Setup**: `Satr-Setup-2.16.21.exe` تحت **Assets**.
...
1. **نزّل `Satr-Setup-2.16.21.zip`** (تحت Assets، بكلمة Setup — لا «Source code») بدل الـ
...
3. **تنزيل مباشر يتجاوز المتصفح كلياً** — الصق في PowerShell:
   ```powershell
   irm "https://github.com/m4u05999/Satr/releases/latest/download/Satr-Setup-2.16.21.exe" -OutFile "$HOME\Downloads\Satr.Setup.exe"
   ```
...
```powershell
(Get-FileHash "$HOME\Downloads\Satr-Setup-2.16.21.exe" -Algorithm SHA256).Hash
```
...
### كيف تدخل بحسابك — وماذا تقول شروط المزوّدين

«سطر» **لا يعرض شاشة تسجيل دخول خاصة به لأي مزوّد، ولا يخزّن اعتمادك، ولا يمرّر طلباتك
نيابةً عنك**. الدخول يتم دائماً بأداة المزوّد نفسها على جهازك، و«سطر» يقرأ حالة الدخول
(مسجَّل/غير مسجَّل) لا أكثر:

| المحرّك | كيف تدخل | ما تقوله شروط المزوّد (قُرئت 2026-09-03) |
|---|---|---|
| **Claude Code** (اشتراك Pro/Max أو مفتاح API) | `claude login` في الطرفية | Anthropic تسمح صراحةً بأن «يسجّل المستخدم دخوله إلى ثنائي Claude Code **غير المعدَّل** باشتراكه، بما في ذلك حين تستضيفه منصة»، بشرط ألا يعدّل المضيف الثنائي ولا يتوسّط الدفع ولا يلمس الاعتماد — و«سطر» يشغّل `claude.exe` المثبَّت عندك كما نشرته Anthropic. وتوصي مطوّري Agent SDK بمفتاح API. ([النص الرسمي](https://code.claude.com/docs/en/legal-and-compliance)) |
| **Codex** (اشتراك ChatGPT أو مفتاح API) | `codex login` أو زر الدخول في ⚙ (يفتح متصفح OpenAI) | وثائق OpenAI تجعل `codex login` المسار الافتراضي، وتوصي بمفتاح API للأتمتة، وتطلب معاملة `~/.codex/auth.json` ككلمة سر — و«سطر» لا يرسله ولا يعرضه. ([المصادقة](https://learn.chatgpt.com/docs/auth)) |
| **Kimi Code** (اشتراك Kimi أو مفتاح API) | `kimi login` في الطرفية | Moonshot تنصّ على أن «الأعضاء يستعملون مزاياهم في العميل الرسمي **والمنصات الخارجية**»، وشرطها الوحيد عدم العبث بمعرّف العميل — و«سطر» يشغّل `kimi acp` الرسمي بلا تعديل. ([وثائق Kimi Code](https://www.kimi.com/code/docs/en/)) |

نقطة صريحة لمشتركي Claude Pro/Max: منذ مايو 2026 يُحاسَب **الاستخدام البرمجي** للاشتراك
من رصيد شهري منفصل لـAgent SDK. هل يُحسب دور «سطر» منه أم من حصة Claude Code العادية؟
لم نقسه بعد، وسنوثّقه هنا حين نفعل.

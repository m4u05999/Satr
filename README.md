...
<p align="center">
  ...
  <img alt="الإصدار" src="https://img.shields.io/badge/version-2.16.0-7FB069">
</p>
...
قسم **«Assets»** ونزّل **`Satr-Setup-2.16.0.exe`** — ثم شغّله (واجهة التثبيت عربية بالكامل).
إن لم يكن Claude Code مثبّتاً، يرشدك معالج أول التشغيل لتثبيته وتسجيل الدخول.

> [!WARNING]
> **لا تنزّل «Source code (zip/tar.gz)»** — هذا أرشيف اسمه `Satr-2.16.0.zip` (بشرطة)
> يحوي **شفرة المصدر** (مجلدات `electron` و`src` وملف `package.json`…) **وليس فيه
> التطبيق**. ملف التطبيق فيه كلمة **Setup**: `Satr-Setup-2.16.0.exe` تحت **Assets**.
...
1. **نزّل `Satr-Setup-2.16.0.zip`** (تحت Assets، بكلمة Setup — لا «Source code») بدل الـ
...
3. **تنزيل مباشر يتجاوز المتصفح كلياً** — الصق في PowerShell:
   ```powershell
   irm "https://github.com/m4u05999/Satr/releases/latest/download/Satr-Setup-2.16.0.exe" -OutFile "$HOME\Downloads\Satr.Setup.exe"
   ```
...
```powershell
(Get-FileHash "$HOME\Downloads\Satr-Setup-2.16.0.exe" -Algorithm SHA256).Hash
```

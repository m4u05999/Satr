...
1. **نسخ العمل ضمن المجلدات الإضافية للجلسة** (‏`--add-dir` أو إعداد الجلسة) كي يغطيها
   `acceptEdits`؛ القواعد وحدها لا تكفي لمسارات خارج مجلد الجلسة. **والصيغة المقيسة
   (2026-09-14/15)**: `"permissions": { "additionalDirectories": ["D:/sater"] }` في
   `.claude/settings.local.json` هي ما أدخل نسخ العمل في نطاق الجلسة فعلاً — بمسار
   ويندوز عادي بشرطات أمامية لا بصيغة القواعد `//d/…`. قبله كانت قواعد السماح تمرّر
   الأمر ويبقى المسار خارج النطاق، فيتوقّف المنفّذ رغم أن قاعدته موجودة.
...
3. **المنفّذون في المقدمة — قاعدة مؤقتة (‏OBS-208)**: طلب الإذن وسؤال النموذج من وكيل فرعي
   **خلفي** كانا يُسقطان بعد انتهاء دور القائد، فيقف المنفّذ على طلبٍ لا يصل مربع الإذن أصلاً —
   ولا تنفعه أي قاعدة سماح. الإصلاح مدموج (`0c2f2d9`)، وتبقى القاعدة سارية **حتى القبول البشري
   له**: أطلق المنفّذين في المقدمة لا في الخلفية.

...
async function start({ prompt, images, sessionId, model, permissionMode, skills, effort, extraDirs, browserControl }, cwd, emit) {
  ...
  options.systemPrompt = {
    ...
    append: '**اللغة**: تواصل مع المستخدم بالعربية افتراضياً في كل شرحك وردودك وملخّصاتك ' +
      ...
      'Gemini) تكتبها سليمة. لذا إن احتاج المستخدم نصاً عربياً داخل الصورة: استعن بـ ' +
      'models_explore لترشيح نموذج قوي في كتابة العربية واستعمله مباشرةً؛ ومع نموذج أضعف ' +
      '(أو حين تريد تحكّماً طِباعيّاً دقيقاً في الخط) ولّد الخلفية/العنصر ثم ضع النص العربي ' +
      'كطبقة HTML/CSS فوقه في الموقع. لا تمنع النص العربي منعاً مطلقاً — بل اختر الأداة المناسبة له.' +
      (portableSkillPrompt ? '\n\n' + portableSkillPrompt : ''),
  };

...
// `gpt-5.6-cyber` (‏2026-08-10) فرعٌ أمنيّ مدرَّب من أوزان Sol نفسها، **محجوب خلف
// تصنيف Daybreak Red**: لا يُزوَّد إلا لحساب مفحوص، ودونه لا وجود له أصلاً.
// مقيس على حساب المطوّر (2026-09-05): يردّ `404 model_not_found` **تماماً كمعرّف وهمي**،
// بينما `gpt-6-astra` و`gpt-5.6-terra` يردّان `429 credit_balance_exhausted` — أي أن
// الحجب على مستوى الحساب لا خطأ في المعرّف. ولذلك **لا يمكن التحقق من قدراته حياً هنا**،
// والحقول أدناه من صفحته الرسمية ومن نسبه إلى Sol:
//   • سياق 400,000 (‏272K إدخال · 128K إخراج) — أضيق من عائلة 1.05M.
//   • سعر 12.5/1.25/75 لكل مليون — الأغلى في القائمة كلها.
//   • مستويات الجهد **غير معدودة في التوثيق**؛ وُرِثت الخمسة عن Sol (المصدر يذكر أنه
//     يستعمل أوزانه) وهي **غير متحقَّق منها**. والتدهور هنا مقصود مرئي لا صامت: لو رفض
//     النموذج مستوى ما، ردّ API صريح يراه صاحب الوصول فوراً — وهو أفضل من إسقاط الحقل
//     بصمت (نمط `OBS-113`/`OBS-121`).
...
const MODEL_CAPABILITIES = new Map([
  ...
  ['gpt-5.6-cyber', { vision: true, effort: new Set(['low', 'medium', 'high', 'xhigh', 'max']) }],

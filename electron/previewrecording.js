...
const SAFE_PROMO_FINAL_NAME = /^satr-promo-final-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.(?:mp4|webm)$/;
...
function recordingSavePath(downloadsPath, filename, exists = fs.existsSync) {
  ...
  if (SAFE_PROMO_FINAL_NAME.test(filename || '')) {
    const extension = path.extname(filename);
    const stem = filename.slice(0, -extension.length);
    for (let index = 1; index <= 999; index += 1) {
      const candidate = path.join(downloadsPath, index === 1 ? filename : stem + '-' + index + extension);
      if (!promocapture.isInsideDownloads(downloadsPath, candidate)) return null;
...
function attach(session, ownerWebContents, options) {
  ...
  const handler = (_event, item, webContents) => {
    ...
    item.once('done', (_doneEvent, state) => {
      ...
      const promoFinal = SAFE_PROMO_FINAL_NAME.test(filename);
      const savedFilename = path.basename(savePath);
      const result = state === 'completed'
        ? { type: promo ? 'promo_recording_saved' : promoFinal ? 'promo_final_saved' : 'preview_recording_saved', filename: promo || promoFinal ? filename : savedFilename, path: savePath }
        : { type: promo ? 'promo_recording_failed' : promoFinal ? 'promo_final_failed' : 'preview_recording_failed', filename: promo || promoFinal ? filename : savedFilename, state: String(state || 'unknown') };
      if (promo || promoFinal) result.saved_filename = savedFilename;
      emit(result);
      onResult(result);
    });
  };
...
module.exports = { SAFE_RECORDING_NAME, SAFE_PROMO_FINAL_NAME, recordingSavePath, attach };

 ...
 */
const METRIC_VERSION = 5; // ‏1 قبل PascalCase · 2 قبل وعي الخط · 3 قبل استثناء صفوف الجداول · 4 قبل استثناء بنود المعرّفات
 ...
 * `script:'ar'` **ولا صفوفَ جداول فيه ولا بنودَ معرّفات** يعطي `share` و`slip`
 * و`reason` مطابقةً حرفياً لـv2؛ ولا يتغيّر إلا حكم الفارسي الخالص (‏v3) وصفوف
 * الجداول (‏v4) وبنود القوائم كثيفة المعرّفات (‏v5).
 *
 ...
 *
 * **وحدُّ v5 معلَن (‏OBS-115)**: استثناء المعرّفات امتدّ من صفوف الجداول إلى
 * بنود القوائم — القاعدة واحدة (لا جملة إنجليزية ⇒ معرّفات مقتبسة) لكن تطبيقها
 * كان حكراً على الجداول، فرُصدت إدانات باطلة مقيسة على بنود حقيقية (أسماء
 * نماذج، فئات استبيانات، وسوم واجهات). صفوف v4 المتراكمة قيست بلا هذا الاستثناء
 * فلا تُقارن أرقامها بأرقام v5 (قاعدة «لا مقارنة عبر إصدارين»). والعناوين نثرٌ
 * بحكمها عمداً (نمط شكوى المالك) ولم تدخل الاستثناء في الإصدارين.
 */
 ...
 * checked |` مُداناً.
 *
 * **امتداد الاستثناء إلى بنود القوائم (‏OBS-115، 2026-09-05)** — مقيس لا
 * مفترض: عيّنة اصطناعية من أنماط ردود «سطر» اليوم أدانت v4 ثلاثة أسطر مشروعة
 * في حالتين: بند أسماء نماذج (`- أسماء نماذج Claude الرسمية: **Fable 5.1**
 * وOpus 5…`) وبند فئات استبيان، وخطوة مشي في واجهة إنجليزية (`1. في **تطبيق
 * Claude**: **Settings** ← **Capabilities**…`). جوهرها عربي وإنجليزيها معرّفات
 * يوجب العقدُ بقاءَها — وهو بالضبط ما عولج للجداول في OBS-057، لكن الآلية
 * (كثافة المعرّفات تُقرأ انزلاقاً) بقيت حيّة في البنود فرصدها النموّ المقيس
 * لمجتمع `structure`. فصار الاستثناء: أي سطر بنيوي **ليس عنواناً** وبلا تتابع
 * ثلاث كلمات إنجليزية في نثره = معرّفات مقتبسة. البند الإنجليزي الحقيقي جملةٌ
 * بطبيعته فيتّهم بها (`- The handler swallows every error silently`)، والعناوين
 * نمط شكوى المالك فتبقى بحكمها العام — الاستثناء لا يمسّها. `METRIC_VERSION = 5`.
 */
...
function structuralSlips(text) {
  ...
  for (const line of text.replace(FENCED_RE, '').split(/\r?\n/)) {
    ...
    if (measured.share !== null && measured.share < 0.5
      ...
      // سطر بنيوي بلا جملة إنجليزية = معرّفات مقتبسة (خلايا جداول OBS-057، وبنود
      // القوائم OBS-115: أسماء نماذج، وسوم واجهات، فئات استبيانات) — الفارق ليس
      // اللغة بل الجملة. العناوين نثرٌ بحكمها (نمط شكوى المالك) فتبقى مُدانة عمداً.
      if (!HEADING_RE.test(trimmed) && !ENGLISH_RUN_RE.test(proseOf(trimmed))) continue;

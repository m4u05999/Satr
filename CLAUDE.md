...
- **⚠️ تحذير upstream جديد رُصد أثناء الجسّ**: `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` ظهر في
  ...
  قِيس بفخّ حيّ (‏`OBS-140`، ‏`npm run probe:obs140`)**: قاعدة `permissions.allow` في
  `.claude/settings.json` للمشروع **لا تظلّل `canUseTool`** — استُدعي المربع في مشهدَي
  `['Write']` و`['Write(*)']` ولم يُكتب ملف الفخّ. وصلاحية النفي مسنودة بشاهد موجب:
  `deny:['Write']` أسقطت الأداة من إعلان `system:init` (‏`73 → 72`) فالملف **مقروء
  يقيناً**. الباقي غير المقيس (إعدادات المستخدم و`settings.local.json` وصيغ
  `Bash(...)`) مذكور في الملاحظة.

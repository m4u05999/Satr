...
electron/genmedia.js ← نواة توليد الوسائط BYOK (م١/ج8): سجل مزوّدين بطبقتين + كتالوج
                       أسعار تقديري مؤرَّخ + توجيه أرخص-أولاً بسقوط صريح + تنزيل الأصول
                       إلى <cwd>/generations/ وسجل JSONL في <cwd>/.satr/generations.jsonl.
...
- **مهارة معرفة سطر الذاتية (satr-guide — قرار «معرفة سطر» 2026-07-18)**: `.agents/skills/
  ...
  علاج التقادم آلي لا بشري. تُحزم `satr-guide` مع `satr-diverge` كمصدري `builtin` بأدنى
...
- **المحتوى**: شبكة بطاقات من سجل v1 — مصغرة كسولة عبر IntersectionObserver (السجل
  ...
  لا يغلق اللوحة). الفيديو بطاقة معلومات بلا معاينة (مؤجل ج9)، والفاشلة تعرض
...
- **الاختبار**: `scripts/gallery-live-test.js` (Electron حي تحت CSP، fixture
  ...
  المشتركة) يغطي: فتح/إغلاق، شبكة 4 بطاقات، مصغرات كسولة، فيديو مؤجل، فاشلة،

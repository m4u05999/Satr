...
- **⚠️ تحذير upstream جديد رُصد أثناء الجسّ**: `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` ظهر في
  ...
  - **والتنبيه منفَّذ في `hookguard.js`** (‏إخبارٌ لا إنفاذ): يقرأ `permissions.allow`
    ...
    `allow` الاختياري بجانب `mcp` (بلا رفع `STORE_VERSION`). يحرسه `test:hookguard`
    بـ12 فحصاً، ومُثبَت أنه يعضّ. **والإنفاذ ما زال ناقصاً**: يحتاج `PreToolUse` hook
    وهو دفعة مستقلة لأنه يمسّ أخطر مسار في التطبيق.

...
- **حدود upstream المثبتة**: القيم الخام كانت `default` و`opus[1m]` و
  ...
  ذواتا الأقواس لا تطابقان `SAFE_MODEL` الحالية، لذلك يعبر IPC ثلاثة نماذج ديناميكية فقط؛
  لا تُوسّع القاعدة لقبول صيغة غير متفق عليها. أعاد `accountInfo()` مفاتيح

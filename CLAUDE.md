...
- **التعافي وكثافة السطح**: الأخطاء العامة تستخدم `failure_code` ثابتاً ورسالة آمنة بلا `stderr`
  ...
  المضيف عنصر flex نسبي داخل `#midRow`، لذلك يبقى محصوراً بين الهيدر والمؤلّف ويستخدم ظل
  ...
  إلى drawer كامل العرض مع `role=dialog` و`aria-modal`، ويختفي السحب والطيّ حتى العودة لسطح المكتب.

...
- **وضوح التشغيل بعد التجربة الحية**: reducer يشتق `nextAction` حتمياً من البوابات نفسها ويبرز
  الزر الموافق بلا تشغيل تلقائي. بطاقات العوامل تعرض آخر أداة مسموحة ومساراً نسبياً منقّى والوقت
  المتبقي وميزانية الكتابة، بلا نسبة إنجاز تخمينية. نموذج البدء يختار مهلة 3/5/10 دقائق، وتتحقق
...
- **التعافي وكثافة السطح**: الأخطاء العامة تستخدم `failure_code` ثابتاً ورسالة آمنة بلا `stderr`
  ...
  اللوحة قابلة لتغيير العرض ولها وضع مصغّر محفوظ محلياً، والإشعارات الداخلية محصورة في نهايات
  التنفيذ والمراجعة والتحقق.
- **العصف والتقسيم الاقتراحي**: تبويب العصف يرسل brief منقّى إلى SDK وCodex بصورة مستقلة ومربوطة
  بالمشروع الحالي؛ لكل

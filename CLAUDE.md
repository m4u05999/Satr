...
- **تذكّر آخر مشروع + تحديث تلقائي (م-1-ج — طلب مالك)**: النقر على 🌐 يفتح آخر عنوان
  ناجح مباشرة (`satr_preview_url`)؛ وزرّ 🔄 «تحديث تلقائي» (مُفعّل افتراضياً) يجعل
  القشرة تستدعي `previewEl.reloadIfLive()` عند اكتمال دور عدّل ملفات (تتبّع
  `previewDirty` على `file_edit`). المكوّن يعيد التحميل فقط إن كان الوضع مفعّلاً
  والعرض حيّاً. حدّ: بعد اكتمال الدور — مشاريع HMR تتحدّث بنفسها (يُفضَّل إطفاؤه لها).

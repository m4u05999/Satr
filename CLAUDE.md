...
electron/executionteam.js ← منسّق 1–3 عوامل منفّذة متوازية؛ worktree وملكية كتابة لكل عامل،
                       كشف تعارض وإيقاف جماعي بلا دمج
...
- **التعارض**: قبل إنشاء أي worktree تُرفض الملكيات المتداخلة تحفظياً بـ`ownership_overlap`؛
  ...
  خريطة الملفات الملموسة، وأي ملف يظهر لدى عاملين يسجّل `same_file` ويوقف الفريق. لا دمج ولا
  commit ولا فرع مستخدم متأثر؛ العقد يصرّح دائماً `merged:false` و`merge_supported:false`.
- **IPC والواجهة**: القنوات المنفصلة `satr:executionTeamStart/Stop/Latest` تتحقق من `cwd`،
  عدد العوامل، المهام، الملكيات، ومعرّف الفريق في `main.js`. الحدث `execution_team_update`
  (schema v1) يغذّي لوحة `/تنفيذ-معزول`: إعداد عاملين افتراضياً حتى 3، بطاقة لكل عامل تعرض
  المهمة والملكية والحالة والكلفة وميزانية الكتابة والفرق، وتحذير تعارض وزر «إيقاف الكل».

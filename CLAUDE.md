...
- **⚠️ تحذير upstream جديد رُصد أثناء الجسّ**: `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` ظهر في
  ...
  قِيس بفخّين حيّين (‏`OBS-140`)، والنتيجة تفترق بحسب موضع الملف**:
  - **إعداد المشروع لا يظلّل** (‏`npm run probe:obs140`): استُدعي المربع في مشهدَي
    `['Write']` و`['Write(*)']` ولم يُكتب ملف الفخّ. والنفي مسنود بشاهد موجب —
    `deny:['Write']` أسقطت الأداة من إعلان `system:init` (‏`73 → 72`) فالملف مقروء يقيناً.
  - **🔴 إعداد المستخدم يظلّل تظليلاً كاملاً** (‏`npm run probe:obs140-user`، بـ`HOME`
    معزول): `canUseTool` **لم يُستدعَ إطلاقاً** في مشهدَي السماح، و**كُتب الملف فعلاً**
    (علامةٌ على القرص + `File created successfully`). أي أن قاعدة سماح في
    `~/.claude/settings.json` **تتخطّى مربع الإذن العربي**. لا يمسّ المالك اليوم (لا
    مفتاح `permissions` في إعداده).
  - **والتنبيه منفَّذ في `hookguard.js`** (‏إخبارٌ لا إنفاذ): يقرأ `permissions.allow`
    من ملف المستخدم عند بدء الدور ويبثّ تنبيهاً عربياً يسمّي الأدوات التي لن تُسأل.
    محصورٌ بملف المستخدم وحده (قاعدة المشروع لا تنبّه — تحذيرٌ عن ملفٍ لا يظلّل كذبٌ
    بالزيادة)، ويأخذ **اسم الأداة بلا وسيطتها**، ويخزّن **بصمة لا قائمة** في حقل
    `allow` الاختياري بجانب `mcp` (بلا رفع `STORE_VERSION`).
  - **✅ والإنفاذ منفَّذ**: **لا خطّاف جديد** — `agent.js` يسجّل `PreToolUse` أصلاً وفيه
    ...
    (`hookguard.userAllowToolNamesSync`) لأن الخطّاف قد يقع قبل حسم قراءة لاحقة.
    مُثبَت على مسار الإنتاج بـ`npm run probe:obs140-enforce`: مع `allow:['Write']` صار
    `permissionRequests:1` ولم تقع الكتابة، وشاهدٌ سالب سليم.
    **تجاوزان معلَنان**: `bypassPermissions` مستثنى (تجاوزه مقصود)، والمطابقة **بالاسم
    ...
    ويغطّي محرك `sdk` وحده لأن التظليل مقيسٌ فيه. يحرسه `test:hookguard` (‏54 فحصاً)،
    ومُثبَت أنه يعضّ.

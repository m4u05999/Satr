...
const SUITE = [
  ...
  'test:enterprise',
  'test:execution-host',
  'test:saved-task-execution',
  'test:saved-tasks-preload',
  'test:saved-tasks-ui',
  'test:usage-summary',
  'test:activity',
...
const EXCLUDED_FROM_SUITE = Object.freeze([
  ...
  { name: 'probe:obs140-scope', reason: 'مسبار قياس خارج الطقم — يستدعي resolveSettings ليقارن ما يصل الإعدادات الفعّالة من نطاقي المشروع والمستخدم (OBS-140)؛ قطعي بلا أدوار لكنه قياسٌ لا حارس' },
  { name: 'probe:obs140-trust', reason: 'مسبار حيّ خارج الطقم — يقيس هل ثقة مساحة العمل (hasTrustDialogAccepted) تفسّر افتراق النطاقين (OBS-140)؛ يستهلك أدوار SDK' },
 ...
 * `0xC0000005` **بلا سطر خرج واحد** — أي أن `node-pty` ينهار قبل أول فحص. أُدرج
 * لأن التواتر مقيس (‏ثلاثة سقوطات من خمسة تشغيلات في يوم واحد: طقم محلي + أول
 * تشغيل لكلا الطلبين #79 و#80 على CI، ونجاحٌ `3/3` منفرداً وبإعادة التشغيل بلا
 * تغيير) — لا لأنه «قد يتعثّر». والإدراج **يُخفي الهشاشة ولا يزيلها**: السبب
 * الجذري لم يُشخَّص، و`OBS-139` تبقى **مفتوحة** بعده. فإن سقط مرتين متتاليتين
...
const SKIP_ON_POSIX = Object.freeze([
  { name: 'test:pty-native-lifecycle', reason: 'سباق ConPTY خاص بويندوز؛ يشغله تحقق ويندوز في release.yml ويقيس خروج Electron الحقيقي' },

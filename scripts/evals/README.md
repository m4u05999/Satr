...
## تحقق الأوامر وcheckpoints

```powershell
npm run test:verify
```

اختبار حتمي بلا شبكة ينشئ `.satr/verify.json` مؤقتاً، يثبت أن التشغيل لا يحدث إلا عند
استدعاء runner، ويرفض الأمر متعدد الأسطر، ثم يختبر حفظ checkpoint والاستعادة العكسية
وحدود `cwd` وعودة نتيجة التحقق اليدوي للمحرك مرة واحدة. لا browser ولا preview ولا Git history.

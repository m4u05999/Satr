...
// ── قابلية المقارنة ─────────────────────────────────────────────────────────
// ── فصل الأسباب بإصدار المقياس (‏OBS-058) ────────────────────────────────────
// عدّ أسباب كل إصدار وحده كي يكون القياس عند اكتمال v4 (‏921) مباشراً بلا حفر
// يدوي بـ`node -e`. أرقام فقط (‏OBS-057) — لا نص رسائل في السجل أصلاً.
const byVersion = new Map();
for (const row of slips) {
  const v = 'v' + (row.v || 0);
  if (!byVersion.has(v)) byVersion.set(v, new Map());
  const reasonsV = byVersion.get(v);
  reasonsV.set(row.reason, (reasonsV.get(row.reason) || 0) + 1);
}
if (byVersion.size > 1) {
  console.log('\nتوزيع أسباب الانزلاق بإصدار المقياس:');
  for (const [v, reasonsV] of [...byVersion.entries()].sort()) {
    const total = [...reasonsV.values()].reduce((a, b) => a + b, 0);
    console.log('  ' + v.padEnd(6) + String(total).padEnd(6)
      + [...reasonsV.entries()].sort((a, b) => b[1] - a[1]).map(([r, n]) => r + '=' + n).join(' · '));
  }
}

// ── قابلية المقارنة ─────────────────────────────────────────────────────────
// القاعدة المجمَّدة: لا تُقارن الأرقام عبر إصدارين. التوثيق الوحيد للتوافق هو حكم
// v3 مع v2 للنثر العربي — أي خلط آخر (‏v2/v4 مثلاً) غير قابل للجمع، وفصل الأسباب
// بإصدار المقياس أعلاه يحدد حجم كل طرف بلا تجميع مضلِّل.
const versions = [...new Set(rows.map((row) => row.v))].sort();
if (versions.length > 1) {
  console.log('\nℹ️ السجل يخلط إصدارات مقياس (' + versions.map((v) => 'v' + v).join(' · ')
    + '). التوافق الموثَّق وحده v3↔v2 للنثر العربي؛ فإن لم يكن الخلط هو هذا بالذات'
    + ' فأرقام كل إصدار تُقرأ من فصله أعلاه ولا تُجمع.');
}

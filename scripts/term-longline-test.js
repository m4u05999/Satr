 ...
 *
 * **قياس بيئيّ** (‏⑥ وحده): مشهد عناقيد الحرف عند حافة الالتفاف. نجاحه يعني «عطل
 * العرض في ConPTY **ما زال قائماً**» — رسالته نفسها تقول «القياس تغيّر: أعِد القياس
 * وحدّث `OBS-106`». فهو **تريّس رصدٍ لا حارس صحّة**، ولا يصلح بوابةً محمولة: سقوطه على
 * آلة أخرى لا يعني تراجعاً. مقيس: أُدرج في بوابة ويندوز فسقط على العدّاء المستضاف
 * (‏`33943136648`) بـ`splitLines=0` — أي أن الانقسام لا يقع هناك أصلاً.
 *
 * الاختيار **معلَن لا ضمنيّ**: الوسم `--portable` (للبشر) أو المتغيّر
 * `SATR_TERM_LONGLINE_PORTABLE=1` (تستعمله خطوة البوابة — يعبر إلى العملية الابنة بلا
 * طبقتَي تمرير) يشغّل المحمول وحده. الافتراضي **يبقى الكامل** عمداً: لو صار المحمولُ
 * الافتراضَ لاحتاج القياسُ البيئي تذكّراً، «وما يُشغَّل بالتذكّر لا يُشغَّل» (درس
 * `suite-coverage`). فيبقى `test:full` على النسخة الكاملة، والبوابةُ وحدها تضيّق.
 * والوضع **يُطبع** في الرأس والخاتمة ومعه مصدره، فوضعٌ لم يصل يُرى ولا يمرّ صامتاً.
 */
...
// و`@( @('a','b') )` ذات المدخل الواحد تُسطَّح فتضيع الوسائط صامتةً. فالمتغيّر يعبر إلى
// العملية الابنة بلا وسيط. أيّهما وصل يكفي، والوضع يُطبع مع مصدره صراحةً.
const PORTABLE_BY_FLAG = process.argv.slice(2).includes('--portable');
const PORTABLE_BY_ENV = process.env.SATR_TERM_LONGLINE_PORTABLE === '1';
const PORTABLE_ONLY = PORTABLE_BY_FLAG || PORTABLE_BY_ENV;
// المصدر يُسمّى لا الوضع وحده: خطوة البوابة تضبط المتغيّر، والبشر يكتبون الوسم — فسطرٌ
// يقول «‏--portable» بينما لم يصل وسمٌ قط يضلّل من يشخّص لماذا اختلف التشغيلان.
const MODE_LABEL = PORTABLE_ONLY
  ? 'محمول فقط (' + (PORTABLE_BY_FLAG ? '--portable' : 'SATR_TERM_LONGLINE_PORTABLE=1') + ')'
  : 'كامل (محمول + قياس بيئيّ)';
...
async function main() {
  ...
  console.log('term-longline: الوضع = ' + MODE_LABEL);
  ...
  if (PORTABLE_ONLY) {
    // ‏`OBS-122`: يُتخطّى **المشهد كاملاً** لا الفحص الواحد. الحدّ يُقاس بأربعة اعتمادات
    // بيئية متزامنة لا واحد: ‏pty بعرض 40 عموداً، وسلوك التفاف ConPTY، والحافظة كمورد
    // عالمي مشترك (‏`OBS-118`)، ونسخة PSReadLine (‏`OBS-106`). تجزئتُه إلى «الفحص الوحيد
    // الذي سقط» تُدخل البوابةَ فحوصاً مقيسةً مرةً واحدة على عدّاء مستضاف — وقياسة واحدة
    // لا تُبنى عليها بوابة.
    //
    // ⚠️ **الفجوة معلَنة لا مسكوت عنها**: يبقى خارج البوابة حارسان حقيقيان —
    //    «لا تُفقد علامة تشكيل عبر الالتفاف» و«الحركات المُلصقة تصل بعد علاج `OBS-106`».
    //    يغطّيهما التشغيلُ الكامل محلياً على ويندوز (‏`npm run test:term-longline`)
    //    وهو داخل `test:full`، فلا يسقطان من كل مسار.
    console.log('  ⏭ متخطٍّ في الوضع المحمول: قياس بيئيّ (نجاحه يعني «عطل ConPTY باقٍ» لا «الكود سليم»).');
    console.log('     غير مغطّى هنا: فقد علامات التشكيل عبر الالتفاف · وصول الحركات المُلصقة.');
    console.log('     يغطّيهما التشغيل الكامل على ويندوز: npm run test:term-longline');
  } else if (!isPwsh) {
  ...
  // الخاتمة تسمّي الوضع: خاتمةٌ لا تميّز المحمول من الكامل تجعل بوابةً ضيّقةً تُقرأ
  // تغطيةً كاملة — وهو «الأخضر الكاذب» نفسه بوجه آخر.
  console.log('\nterm-longline: نجح — ' + checks + ' فحصاً · الوضع ' + MODE_LABEL
    + ' (حفظ الأسطر، إقلاع السكربت في وسائط spawn، فشل صريح بلا علق، وقصّ ذيل معلَن،'
    + ' وبروتوكول الالتقاط تحت bash/sh نقيّاً'
    + (PORTABLE_ONLY ? '؛ وقياس عناقيد الحرف عند حافة الالتفاف **متخطّى**).'
      : '، وعناقيد الحرف عند حافة الالتفاف مقيسة).'));
  process.exit(0);
}

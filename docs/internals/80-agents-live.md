...
- **الشارات الثماني** بترتيب أسبقية مقصود — الانتظار أعلى الكل (الوكيل متوقّف فعلاً
  ...
  | — | **الحسم المحلي لا يُعيد حسم صفٍّ محسوم**: `finished{local:true}` يُبثّ لكل مهمة رآها Query عند انتهائه، فيصل لوكيلٍ أُنهي صفّه قبله ويقلب «اكتمل» إلى «انتهى مع الدور» — تراجعٌ في الدقة لا تحديث. أما الخاتمة **الحقيقية** (بلا `local`) فتُطبَّق دائماً ولو على صفٍّ منتهٍ: القبول الحي (2026-09-15، `D:\sater\agents-live-shot\live-evidence.json`) أثبت أن `updated{completed}` يسبق `task_notification` فكان رفضُها يُسقط الملخّص الختامي. (مراجعة القائد على PR #161 ثم القبول الحي) |
  | — | **عدّاد الاستئناف من الصف لا من العلم**: الاستئناف بـ`SendMessage` يصل من Query **جديدة** لم ترَ البداية فيأتي `resumed:false`؛ الصف يعدّ بداية ثانية لمعرّف قائم (منتهٍ أو بمعرّف أداة مختلف) استئنافاً. (القبول الحي 2026-09-15) |

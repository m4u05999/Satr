...
- **سطر الإدخال (المرحلة 8.3)**: حقل HTML «عابر» أسفل اللوحة (`#termInput`) — يُفرَّغ عند
  ...
  `\x03` ويفرّغ الحقل **بلا تحديد**؛ مع تحديد في الحقل أو خرج التبويب النشط ينسخ ولا يقطع.
  ينطبق ذلك على عرضَي BiDi وxterm؛ فشل الحافظة يعرض تنبيهاً ويبقي الأمر حيّاً.
  Ctrl+Z/D خامان؛ ▲▼ تاريخ الصدفة والحقل فارغ؛ Tab محجوب (حدّ موثّق)؛
...
- **البناء (حرج)**: `npmRebuild: false` في package.json — node-pty 1.2.0-beta.15 يشحن prebuilds
...
## سباق خروج ConPTY — 2026-09-22

ثُبّت `node-pty` على الرقم الدقيق `1.2.0-beta.15`، وهو إصدار تجريبي مقصود:
آخر مستقر في السجل وقت القياس `1.1.0` يحتوي سباق وصول غير متزامن إلى
`ptyHandles`. ست طرفيات PowerShell تنتظر حاجزاً واحداً ثم تخرج مع تغيير الحجم
أعادت انهيار Electron بـ `0xC0000005`. التفريغ المحلي ورموز الحزمة ربطاه بـ
`SetupExitCallback` في `conpty.cc:106`، عند إزالة العنصر المشترك.
الإصلاح الرسمي [microsoft/node-pty#922](https://github.com/microsoft/node-pty/pull/922)
يضيف قفل الوصول إلى المجموعة، ودخل النسخة المثبتة. لا تبعية إضافية ولا تعطيل للعزل.

`test:pty-native-lifecycle` يشغّل `electron/term.js` الفعلي في Electron معزول:
12 موجة × 6 طرفيات، يتحقق من الخرج ورموز الخروج واستمرار تغيير الحجم، ثم يشهد
الأب خروج Electron نفسه. دخل الطقم وبوابة ويندوز. السباق احتمالي؛ نجاح الجولة
لا يثبت استحالة كل عطل أصلي. خيار `--pty-package` محصور تحت `dist` لإثبات العضّة
بنسخة قديمة دون استبدال اعتماد الإنتاج. أدلة الزرع والمقارنة في
`docs/REPEATED-STOPS-2026-09-22.md`.

القياس المحلي على Windows 10 build 19045 وElectron 33. النسخة التجريبية أزالت
winpty وتعتمد ConPTY؛ لا تعني هذه النتيجة دعماً مثبتاً لإصدارات ويندوز الأقدم.
تفريغ الانهيار التاريخي الأول غير موجود، لذلك لا ندعي تطابق عنوانه مع الانهيار
المعاد؛ المثبت هو إعادة إنتاج عطل الخروج المتزامن وتعيين موقعه واختبار إصلاحه.

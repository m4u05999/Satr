...
**تصحيح نطاق المطابقة — 2026-09-23:** الفحص المحلي للخطّافات يجمع `SessionStart`
في `.claude/settings.json` و`.claude/settings.local.json` فقط، بينما الطرف الآخر
كان يُدخل جميع أحداث المحرّك وكل المصادر. لذلك كان `PreToolUse` يولّد تنبيهاً متكرراً
عن فرق سببه تغطية الفاحص. المطابقة الآن تقصر طرف المحرّك على الحدث والنطاقين
المفحوصين؛ الباقي غير مفحوص، ولا يعني غيابه من الفروق أنه آمن أو غير موجود.
`agreed` يعني عدم وجود فرق ضمن التغطية فقط. أخطاء إعداد المحرك، وفروق `SessionStart`
الحقيقية، ومطابقة قواعد السماح تظل فعالة. حارس الملف الحقيقي ذي `PreToolUse` صامت،
وحارس اختلاف `SessionStart` ينبّه؛ عضّة إزالة مرشّح التغطية سقطت فعلاً.
راجع [تقرير الإصلاح](../CONTINUITY-REPAIR-2026-09-23.md).

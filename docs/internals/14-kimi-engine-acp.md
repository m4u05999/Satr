...
- **تصنيف رفض Kimi بنصّه لا برمزه (‏OBS-189، 2026-09-12)**: كان موضعا `catch` حول `initialize`
  و`session/prompt` يترجمان كل خطأ JSON-RPC برمز `-32000` إلى «غير مسجَّل الدخول» ويرميان
  `error.message` الذي تحفظه طبقة RPC، فبدا نفاد الحصة بعد استعمال K3 الكثيف عطلَ دخول لا يحلّه
  `kimi login` ويزول وحده بعد يومين. الآن `describeRpcFailure(error, phase)` (نقية، مُصدَّرة في
  `_internals`) تصنّف بالنصّ: `auth` (أنماط الدخول أو نصّ فارغ مع `-32000` — الافتراض القديم محفوظ)،
  `quota` (‏`quota|rate limit|429|too many|exhaust|balance|capacity|overload|503`) برسالة تنصح
  بالانتظار أو تبديل النموذج لا بإعادة الدخول، و`rpc` لما سواهما بالرمز والنصّ. النصّ يصل محجوب
  الأسرار (‏`scrubError`) ومقصوصاً، ويُبثّ `spawn_error` بحقل اختياري `kind` تستعمله القشرة لعرض
  النصّ الأصلي بدل تلميح «مثبت ومسجّل دخوله» حين لا يكون الرفض دخولاً. و`recordEngineError()` تكتب
  سطراً دائماً في `~/.satr/engine-errors.log` (طابع · مرحلة · تصنيف · رمز · نصّ محجوب، سقف 512 ك.ب)
  لأن سجل kimi-code نفسه توقف قبل العطل فلم يبقَ أثر. **حدّ**: نصّ رفض الحصة الفعلي لم يُقَس بعد؛
  أول ظهور يكتبه السجل فتُطابَق الأنماط عليه. `test:kimi` يغطي التصنيفات الثلاث وحجب السرّ.

- **تشخيص خروج العملية ومسار المثبّت الرسمي (‏OBS-215/217/218/219، 2026-09-17 — بلاغ مستخدم
  على ويندوز 10 وKimi 0.43.1)**:
  - **OBS-217 — خروج العملية يحمل تشخيصه**: كان `proc.on('exit')` يغلق القناة بـ`new Error('Kimi exited')`
    فيصل المستخدم النصّ العاري ويُكتب في `engine-errors.log` كذلك؛ وكان `onProcExit` يُنادى متزامناً
    فيسبق الرفضَ بصندوق «أُنهيت عملية Kimi Code (كود N)» ثم يتلوه صندوق ثانٍ. الآن: `createRpc` يتتبّع
    **المرحلة** (`rpc.phase()` = الطلب المعلّق الأحدث وإلا آخر طلب أُرسل: `initialize` · `session/new` ·
    `session/resume` · `session/load` · `session/prompt` …)، والقناة تحتفظ بـ**ذيل stderr**
    (`createStderrTail`: آخر 20 سطراً/2 ك.ب، كل سطر ≤400 محرف، محجوب بـ`scrubError`)، و`processExitError(code,
    signal, phase, tail)` يبني الخطأ بحقول `exitCode/signal/phase/stderrTail/processExited` ونصّ عربي
    «خرجت عملية Kimi أثناء <المرحلة> (رمز الخروج N | إشارة S)» + الذيل أو «لم يكتب Kimi شيئاً على stderr».
    `describeRpcFailure` يصنّفه `kind:'exit'` ويمرّر النصّ كما هو (لا يُصنَّف بأنماط الدخول/الحصة كي لا
    يحرف سطرٌ عابر في stderr التشخيص) مذيّلاً بـ«التفاصيل محفوظة في ~/.satr/engine-errors.log»؛ و`recordEngineError`
    يكتب المرحلة الدقيقة و`exit=N` بدل `code=-`. `onProcExit` صار في `setImmediate` فيحسم الرفضُ `finished`
    أولاً ويبقى هو ضامن `proc_done` فقط — صندوق واحد. الموضع الثاني (`withProbe`) يحمل الذيل نفسه.
    الحارس (`test:kimi` ← `testProcessExitDuringInitCarriesDiagnostics`): عملية وهمية تكتب إلى stderr على
    دفعتين (سطر مقطوع + سرّ Bearer) وتخرج بـ`3221226505` أثناء `session/new`؛ يثبت أن النصّ والسجلّ يحملان
    المرحلة والرمز والذيل محجوباً، وأن `spawn_error` واحد. السجلّ يُحقن للحارس عبر `create({ engineErrorsLog })`.
  - **OBS-215 — مسار المثبّت الرسمي**: `kimiBinCandidates()` (مستخرجة من `resolveKimiBin`) تضع
    `<dataRoot()>/bin/kimi.exe` — أي `~/.kimi-code/bin` أو `KIMI_CODE_HOME` — أول المرشّحين بعد `KIMI_BIN`،
    فيُحلّ المحرّك ولو فشلت إضافة Path (رفض كتابة `HKCU\Environment` في جهاز المستخدم) أو لم يُعَد تشغيل سطر.
    رسالة «لم يُعثر على Kimi Code CLI» تسرد المرشّحين وتذكر `KIMI_BIN`/`KIMI_CODE_HOME`؛ ورسالة الدخول صارت
    `loginHint(bin)`: **المسار الكامل المحلول** بصيغة `& "<bin>" login` على سطر مستقل (في جهاز المستخدم فشل
    `kimi login` المجرّد بـ`CommandNotFoundException`). `describeRpcFailure(error, phase, bin)` تأخذ المسار
    ثالثاً؛ بلا مسار تبقى العبارة العامة. الحارس `testOfficialInstallDirCandidate`: الترتيب، واحترام
    `KIMI_CODE_HOME`، والحلّ الفعلي من مجلد وهمي بلا Path، ونصّا الرسالتين.
  - **OBS-219 (أ) — زر نسخ كتلة الكود**: `stripBidiControls` في `chat.js` ينزع
    `U+061C/U+200E/U+200F/U+202A–202E/U+2066–2069` من المنسوخ (كتلة الكود وحدها؛ نص الرسالة يبقى — فيه
    عزل مقصود حول أسماء الملفات). القياس حيّ في `test:chat-rtl`: الحافظة مستبدَلة بمصيدة والنقر على الزر
    الحقيقي؛ والعقد البنيوي في `assertStaticContract`. **درس تنفيذ**: أداة التحرير كتبت المحارف خاماً بدل
    `\uXXXX` مرتين (نظير درس BOM)؛ عُدّت بـ`RE.test(source)` قبل الالتزام.
  - **OBS-218 — القياس (بلا كود)**: مسبار خارج المستودع (`D:\sater\kimi-acp-probe-218.js`) يبقي stdin
    مفتوحاً ويرسل `initialize` (بقدرات fs نفسها) ثم `session/new` — مرة `mcpServers:[]` ومرة بخادم HTTP
    وهمي بترويسة `Authorization` يردّ `-32601` — ثم `session/prompt` قصيراً. **النتيجة على جهاز المالك
    (ويندوز 10 19045، Kimi 0.42.0)**: الحالات الثلاث نجحت؛ Kimi اتصل بالخادم الوهمي (طلب MCP `initialize`
    بترويسة Authorization) وأنشأ الجلسة رغم الرفض، وبقي حياً 4 ثوانٍ بعد آخر ردّ وخرج بـ`0` فقط بعد إغلاق
    stdin، وصفر stderr. **⇒ المشتبه الأول (اتصال Kimi بخادم سطر عند الإنشاء) مستبعَد على 0.42.0**؛ وقدرات
    fs مستبعَدة معه (أُرسلت). **حدّان**: إصدار المستخدم 0.43.1 لم يُجرَّب (لم يُرقَّ Kimi المالك بلا قراره)،
    و`session/resume` لم يُقَس. **مُلاحَظ بالمناسبة**: `~/.satr/engine-errors.log` عند المالك يحمل 14 سطر
    «Kimi exited» في مرحلة `prompt` (2026-09-14/15، Kimi 0.42.0، سطر 2.17.0) — أي أن الخروج يقع هنا أيضاً
    لكن في مرحلة أخرى؛ بعد هذه الدفعة سيحمل كل سطر جديد الرمز والذيل. التالي: طلب لقطة الرسالة الجديدة
    ومحتوى السجلّ من المستخدم، ثم إعادة المسبار على 0.43.1.
    **تجربة حية على مسار Satr الكامل** (satr-live-test، `live_b01a4ebef4c27cdeaa630b63`، 2026-09-17): نسخة
    معزولة من هذا الفرع، دخول Kimi داخل بيت التجربة (`KIMI_CODE_HOME` المعزول + OAuth بيد المالك)، خمسة أدوار
    قصيرة من مؤلّف الإنتاج عبر `scripts/lib/live-test-client.js` — الخمسة اكتملت (الأول بإنشاء الجلسة وMCP،
    والباقي على القناة المستأجرة)، صفر أخطاء ولا `engine-errors.log`. لا إعادة إنتاج على 0.42.0. درسان: (١)
    البوابة لا تعيد الفحص بعد دخول جرى بعد الإقلاع فيعود `send()` صامتاً — انقر `.recheck` من السائق أولاً
    (OBS-222)؛ (٢) `lang_slip` يظهر على ردّ من كلمة واحدة `تمام1` (OBS-221).

...
- **إضافة Opus 5.5 (2026-09-22، ‏CLI ‏`2.1.280`، ‏SDK ‏`0.3.270`)**: طلب المالك يوم صدور
  النموذج. المقيس: على `2.1.278` يردّ `claude --model claude-opus-5-5 -p` بخطأ
  `claude_code_version_too_old` (المطلوب `2.1.280`) لا «نموذج مجهول» — فالترقية عبر
  `npm i -g @anthropic-ai/claude-code@latest` هي الإصلاح، لا تعديل كود. بعدها قُبل المعرّف
  الصريح (`canonicalModel: claude-opus-5-5`، نافذة `1000000`، خرج `128000`)، و**المستعاران
  `default` و`opus[1m]` صارا يحلّان إلى `claude-opus-5-5[1m]`** — فقائمة `supportedModels()`
  الست لم تتغيّر نصّياً لكن `officialClaudeName` يعرض الخيارين «Opus 5.5» تلقائياً (المعرّف
  بمقطعين رقميين يمرّ التعبير `((?:-\d+)+)` بلا تعديل — فحص جديد في `test:claude-models`).
  المعدَّل كود: القائمتان الثابتتان الاحتياطيتان (`src/ui/app.js` كانت تحمل `claude-fable-5`
  البائت، و`electron/adapters/index.js`) كسبتا `claude-opus-5-5` صريحاً كي يبقى ثابتاً حين
  ينتقل المستعار `opus` إلى نموذج أحدث. **لم يُمسّ**: `SAFE_MODEL` (يمرّ المعرّف كما هو)،
  ونسخة SDK `^0.3.270` (الأحدث `0.3.280`) — قرار مالك مستقل. **وافتراضي غرفة العمليات**
  بُدّل بقرار مالك في الدفعة التالية مباشرةً (2026-09-22، بعد دمج #167): `resolveOpsRoomModel`
  في `main.js` يعطي SDK ‏`claude-opus-5-5` بدل `claude-opus-4-8`، ويثبّته `test:orchestrator`.
  **حدّ مُصرَّح به**: عوامل غرفة العمليات على جهاز بـCLI أقدم من `2.1.280` تفشل
  بـ`claude_code_version_too_old` عند أول دور؛ المخرج التشغيلي `SATR_OPSROOM_CLAUDE_MODEL`.

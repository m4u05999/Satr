...
- **مساعد إعداد المنصات والتكاملات (2026-07-19)**: `browserpolicy.js` طبقة مستقلة بعد
  ...
  **كل مرة** بلا «دائماً» ولا موافقة دور؛ `bypassPermissions` وحده يتجاوزها. التنقّل أو
  ...
  كل طلب مستخدم جديد 40 فعلاً مؤثراً تُمدّد صراحةً 20 فعلاً. الموافقة الموسعة في طلب مركّب قد تثق
...
- **شفافية المتصفح المشتركة (دفعة «المتصفح عضو مشترك»)**:
  - `click/type/select/hover/scroll/press` تومض outline ذهبياً **داخل الصفحة** قبل الفعل.
    ...
    `contenteditable` عبر focus+selection+execCommand، بحدث الإدخال الأصلي وحده.
...
- الواجهة تستقبل المعرّف من main وتطلب `getUserMedia` بقيود
  ...
  يبقى في رأس المعاينة؛ في مسار منتج الويب يوقف الإيقاف/انتهاء الدور/إغلاق التطبيق المسارات
  ويغلق نافذة المنتج. التنزيل محلي في Downloads باسم `satr-promo-segment-*` منقّى وفريد ولا يُرفع.
...
- **تسجيل نافذة الاختبار الأصلية (2026-09-09)**: `promocapture.startAppWindow(window, options)`
  قدرة داخل main فقط؛ لا IPC عام جديد ولا تمرير `HWND/sourceId` من renderer.
  تتأكد من أن النافذة هي مالكة `ownerWebContents` ومرئية، وتلزم `confirmed:true`
  ومعرّف تجربة وسيناريو صالحين، وترفض الصوت والميكروفون. يستخدم المسار نفسه
  `target_kind:app-window` و`aspect:native` مع العرض والارتفاع الفعليين.
  `prepareAppRecording(videoTrack, pixels)` في `src/ui/lib/media-recorder.js` يستقبل
  `pixels:{width,height}` لمساحة العرض بالبكسلات الفعلية: يحسب المستدعي كل بعد من
  `window.innerWidth/innerHeight` مضروباً في `devicePixelRatio` ومقرباً بـ`Math.round`.
  تُرفض أبعاد الطلب غير الصحيحة أو الخارجة عن `1..10000` بـ`bad_capture_dimensions`
  قبل طلب الالتقاط. يطلب `resizeMode:{exact:'none'}` مع `width/height:{exact:...}`
  وينتظر التطبيق، ثم يضبط `contentHint:'detail'` ويربط المسار بعنصر فيديو منفصل وصامت.
  ينتظر `play()` ثم `requestVideoFrameCallback` بمهلة واحدة `3000ms` تغطيهما معاً؛
  يقرأ بعدها `track.getSettings()` للتحقق من `resizeMode:'none'` وصحة أبعاد التقرير،
  ويعتمد `video.videoWidth/video.videoHeight` من أول إطار في `capture` وحساب معدل البت
  قبل إعلان الجاهزية. يتحقق من جميع الأبعاد ضمن `1..10000`؛ لا يعتمد تقرير المسار
  قبل الإطار، فقد يعلن `1164×761` بينما الإطار والفيديو `1164×760`.
  `none` وحده لا يكفي: قد يختار الالتقاط حجماً أكبر لنافذة ذات إطار؛ تلزم أبعاد مساحة
  العرض الصريحة أيضاً. لا يستبدل المقاس المقيس بأبعاد الطلب.
  ينظف عنصر القياس بـ`pause()` وفصل `srcObject` وإلغاء callback المعلّق والمهلة،
  دون إيقاف مسار الفيديو المملوك للمسجّل، سواء نجح القياس أو فشل.
  فشل الالتقاط يرجع رمزاً منقّى `native_capture_unavailable`، والأبعاد الفاسدة
  `bad_capture_dimensions`.
  أفضلية الترميز لهذا المسار وحده: MP4/H.264 High 4.2 (`avc1.64002A`) ثم High 5.1
  (`avc1.640033`) عند الدعم، ثم `pickRecMime()` القائم؛ اختيار تسجيل الويب لا يتغير.
  معدل البت المطلوب يُحسب من البكسلات عند `30fps` بمعامل `0.4`، بحد أدنى `24Mbps`
  وأقصى `80Mbps`؛ هو طلب إلى `MediaRecorder` وليس ضماناً لمعدل الفيديو الفعلي.
  عقد الجاهزية `promoCaptureReady(sessionId, ok, error, capture)` يمرر
  `capture:{width,height}` من أول إطار؛ يلزم `app-window` مع `ok:true`، ويتحقق main من
  كون البعدين عددين صحيحين موجبين ضمن `1..10000`، ثم يحفظهما في سجل المقطع.
  استدعاء تسجيل الويب القديم بلا `capture` يبقى صالحاً.
  الـrenderer يكتب مسار الفيديو وحده حتى لو كان ميكروفون المعاينة مفعّلاً؛ لا يفتح المعاينة
  أو يغيّر عنوانها أو حجمها. انتهاء دور المحرك أو إيقافه لا ينهي هذا التسجيل أو يغلق نافذته؛
  `stopAll({scope:'run'})` يبقيه جارياً حتى الإيقاف الصريح أو إغلاق التطبيق.
  علم الإطلاق `--recording-hd` يجعل نافذة التجربة ملء الشاشة، ويتطلب شاشة بأبعاد
  أصلية فعلية لا تقل عن `1920×1080`، وإلا يفشل بـ`native_full_hd_unavailable`.
  لا يغيّر وضع الشاشة ولا يكبّر فيديو منخفض الدقة ولا يصطنع `4K`.
  `--smoke --recording-hd` يفك إطارين من الفيديو المحفوظ، ويتحقق من تطابق أبعادهما
  مع السجل وبلوغ `1920×1080` على الأقل، ومن تباين خطوط بعرض بكسل فعلي واحد `>=180`.
  قياسات الجودة الجديدة في `docs/RECORDING-QUALITY-EVIDENCE.md`؛ دليل `1180×800`
  السابق أثبت مسار التسجيل، لكن المالك رفض جودته، فلا يُعد دليلاً على Full HD.
  **حد القشرة**: `MediaRecorder` يعيش في renderer الرئيسي؛ إعادة تحميل قشرة سطر تفقده،
  وقد يصل الإيقاف اللاحق إلى `download_timeout`. لا يوجد مسجّل خارج القشرة يضمن مقطعاً
  متصلاً عبر إعادة تحميلها أو انهيارها. احفظ المقطع قبل إعادة تحميل القشرة، واختبرها دون
  تسجيل مستمر أو صوّر قبلها وبعدها بمقطعين مع إذن متاح للمقطع الجديد. إعادة تحميل المعاينة
  أو PWA المعروضة داخلها تبقي القشرة والمسجّل قائمين، ويمكن تصوير ذلك السطح أثناء التحميل.
  مشغّل المختبر يستهلك `--record-consent` لتسجيل واحد بعد إذن صريح في الجلسة، بلا إعادة سؤال
  عن إذن قائم؛ دون العلم أو بعد استهلاكه يظهر تأكيد أصلي. يبقى الأصل الصامت في
  `dist/live-tests/<id>/home/Downloads`. يعيد `record-stop` الحجم والمدة والأبعاد والبصمة
  مع `scenarioResult:not_evaluated`، وتظل نتيجة السيناريو مستقلة عن حفظ المقطع.
  تُنسخ المقاطع المختارة بعد فحصها فقط إلى Downloads الخاص بالمالك عند طلب الاستوديو،
  ثم يُقترح الخط الزمني؛ لا تصيير تسويقياً قبل طلبه واعتماد المالك للخط الزمني.
...
- تحقق `test:promo-studio` الحي تحت CSP الصارمة يقرأ مقطعين محليين ثابتين، يغيّر ترتيبهما ومدتهما
...
- **تثبيت مدخلات الحارس (2026-09-19)**: كان توليد أحد مقطعي الاختبار نفسه يعطي Blob صفرياً
  أحياناً قبل وصوله للمصيّر. استُبدل بملفي H.264 محليين، مدة كل منهما ثانيتان؛ مصادرهما
  وأوامر توليدهما وبصماتهما في `scripts/fixtures/promo-studio-clips.README.md`.
  يتحقق الحارس من الحجم وSHA-256 ثم فك الفيديو ومدة لا تقل عن 1.5s. تبقى عملية التصيير
  وMediaRecorder ومزج الصوت وفحوص المنتج حقيقية. ينظف finally النافذة ومجلد WAV المؤقت
  عند النجاح والفشل. لا ffmpeg وقت الاختبار أو في الحزمة، ولا تغيير لكود الإنتاج أو المهل أو الإعادات.
  عضّتا الملف الصفري والمدة القصيرة أسقطتا الحارس ثم استعيدت البصمات؛ الأدلة في
  `dist/promo-fixture-review`، وتفصيل إعادة الطقم وحدودها في مواصفة برو §20.
...
- **مقاس العرض تحت أعين الوكيل (‏OBS-175 — الدفعة و، 2026-09-12)**: مستطيلٌ **سالب** العرض
  من الواجهة كان يمرّ في `effectiveBounds` عبر `Math.max(1, …)` فيصير عرضاً حقيقياً بكسلاً
  واحداً، فتعود قياسات `browser_evaluate` كلها بـ`innerWidth=1` بينما اللقطة سليمة. صار
  السالب يُعامَل كالصفر (حجب)، و`setViewport` ينتظر تقارب المقاس بدل `setTimeout(80)` ثابت.
  التفصيل والأرقام في [`74-preview-viewport-evaluate.md`](74-preview-viewport-evaluate.md).
- ⚠️ **`requestAnimationFrame` لا يُطلَق في نافذة مخفية** (‏`show:false` — عُرف اختبارات
  المعاينة كي تعمل بلا شاشة)، فأي انتظار له بلا مهلة **يعلّق العملية إلى الأبد**؛ وكذلك لا
  يُبلَّغ المُصيِّر بتكبير مستطيل العرض فيبقى `innerWidth` عند المقاس الأصغر السابق (قيس: لا
  تقارب خلال 3000ms مخفياً مقابل 0ms ظاهراً). سابق مقيس في `scripts/preview-viewport-evaluate-test.js`.

### هوية متصفح المعاينة والتوافق مع المواقع (Paymob — 2026-09-18)

- قبل إنشاء WebContentsView تضبط هوية persist:preview بواسطة preview-user-agent.js:
  نحذف رمزي Satr/ وElectron/ فقط ونحافظ على إصدار Chromium الحقيقي والمنصة.
  المصدر app.userAgentFallback الأصلي؛ لا نغيره ولا نغير defaultSession أو طلبات التطبيق الداخلية.
- لاستعادة الهوية القديمة شغل التطبيق مع SATR_PREVIEW_NATIVE_UA=1، ثم أعد إنشاء المعاينة.
  لا نمس التخزين؛ تغيير الهوية قد يبطل بيانات موقع يربطها بالبصمة.
- نترك Client Hints الأصلية من Chromium، ولا ندعي Google Chrome ولا نحقن navigator.userAgentData.
  الموقع يستطيع معرفة أنه متصفح Chromium؛ حذف رمزي المنتج غرضه توافق UA فقط.
- هذا حل توافق لا ضمان لكل جهاز: بصمة Paymob تعتمد على عوامل غير UA، واحتمال الإزاحات
  الخطرة لا يساوي نسبة المتصفحات المتضررة. يقاس ناتج البصمة على الجهاز قبل تجربة الدخول.
- test:preview-user-agent يغطي الدالة وفخ UTF-16 بعينة اصطناعية، ومسار preview.open الفعلي:
  أول طلب وnavigator.userAgent والعزل والتخزين واستعادة الهوية وعدم تغيير التطبيق.
  عند secret=56238 قد ينجح JSON.parse رغم تلف الرمز؛ المقارنة بالقيمة الأصلية إلزامية.
  القيمة 656 عينة سليمة وليست ضماناً لبصمة كل جهاز.
- التحقق البشري: محاولة دخول واحدة يجريها المالك، بلا قراءة أو تسجيل كلمات مرور أو رموز.
  لا ندعي إثبات رأس Authorization من سجل المعاينة؛ التسليم البشري يوقف تسجيل الشبكة ويمسحه.

### حجب روابط OAuth من أدوات القراءة — 2026-09-20

أثناء عودة تسجيل لوحة Supabase عبر GitHub، أعادت browser_snapshot عنوان sign-in-mfa وفي fragment رموز اعتماد فعلية. لا تحفظ قيمها هنا أو في أدلة الإصلاح. لم يثبت اكتمال الدخول أو إنشاء مشروع؛ تحذير قفل Auth في console لا يثبت سبب التعثر. سحب المالك تفويض Supabase من GitHub وثبت اختفاؤه من Authorized OAuth Apps. هذا لا يثبت إبطال جلسة Supabase المستقلة أو refresh_token الخاص بها؛ تبقى هذه خطوة متابعة قبل إعادة المحاولة.

الإصلاح في المصدر: browserpolicy.safeOutputUrl يحجب query/hash كاملة عند وجود access_token أو refresh_token أو provider_token أو provider_refresh_token أو id_token أو token أو code أو state، مع معالجة اختلاف حالة الاسم والترميز وhash route وusername/password. العنوان غير القابل للتحليل يحجب؛ العناوين العادية تبقى حرفياً. formatPage يطبق الحجب على عنوان الصفحة ووجهات الروابط؛ formatSnapshot موحد بين agent.js وcodexmcp.js. لا تغيير لعنوان التنقل التشغيلي أو origin أو contextIsolation أو sandbox أو IPC.

نفذ gpt-5.6-sol الرقعة الأولى، وأكمل gpt-6-astra مراجعة الأمان وحالات الحدود وحارس MCP. اختبارات المصدر المصطنعة: browserpolicy 42 وcodexmcp 189، كلاهما exit=0. يمر الحارس عبر خادم MCP وtools/call فعليين مع preview مزيف؛ ليس تسجيل OAuth حقيقياً أو إثبات حزمة. ربط SDK بالصياغة المشتركة فحص بنيوي، وليس تشغيل Claude جديداً. زُرع تجاوز الحجب فسقط الحارسان exit=1 ثم استعيدت البايتات وبصمة SHA-256 وعادا أخضرين. الأدلة dist/oauth-url-review/REPORT.md وbite.json والسجلات؛ syntax وgit diff --check نجحا.

حدود الإصلاح: حجب حقول URL المحددة فقط، لا تنقية شاملة لنص DOM أو عناوين العناصر أو bodyText أو الصور أو console/network أو سجل العناوين أو نتائج بقية الأدوات. لا حذف للأثر السابق ولا ادعاء إبطال رموزه. التطبيق المفتوح والمثبت لم يحدثا ولم يبن إصدار جديد؛ لا تستأنف OAuth في النسخة المفتوحة على أساس هذه الرقعة. آخر الطقم الكامل 147/148 كما سبق، ولم يعد في هذه الدفعة.

## النوافذ المنبثقة داخل المعاينة (2026-09-21)

يسمح معالج window.open بإنشاء WebContentsView ابن عبر createWindow الأصلي في Electron.
لا يستبدل loadURL في الأب. تبقى علاقة opener وpostMessage وWindow.closed تحت سياسات
Chromium؛ noopener وCOOP قد يفصلانها عمداً ولا نتجاوزهما. close_preview يغلق الشجرة كلها،
وزر «إغلاق النافذة المنبثقة» يرسل previewAction('popup_close') لإغلاق الابن النشط وحده.

electron/preview-popups.js يملك ثلاثة أبناء متزامنين كحد أقصى ومهلة عشر دقائق بلا إدخال
ملتزم من المستخدم لكل ابن. الأبعاد الافتراضية 500×650، وتُحصر features والموقع داخل
المساحة المتاحة للوحة؛ browser_set_viewport يتجاوز مقاس الابن ضمن حدود اللوحة نفسها.
الصفحة الأم والأبناء غير النشطين يأخذون حدوداً صفرية، وحجب المعاينة يحجبهم جميعاً.
يبقى Session الأب نفسه، مع sandbox وcontextIsolation وwebSecurity ومن دون node أو preload.
لا نحقن opener ولا ننشئ BrowserWindow للموقع ولا نتجاوز سياسات مزوّدي الهوية.

حدث satr:preview من نوع popup يحمل active/count/origin/secure/warning؛ origin أصل بلا
مسار أو query أو fragment. secure يعني بروتوكول HTTPS، وليس شهادة إتمام مصادقة أو ربط.
الواجهة تعرض شريط الطفل والتحذير خارج مساحة العرض الأصلي، كي لا يحجبه WebContentsView.

currentWC يحلّ الابن النشط تلقائياً لجميع أدوات المعاينة. تبدّل الهدف يبطل refs والبصمات
وينظف سجلات Console/الشبكة وقيم النقل السري، لكنه لا ينهي handoff البشري.
targetToken يجمع هوية WebContents ومراجعة تتغير عند التبديل والتنقل؛ غلافا Codex وSDK
يرفضان موافقة انتظارٍ إن تغيّر هذا الرمز قبل عودة الإذن. نتائح الأدوات تبيّن الهدف المنبثق
والتحذير المتاح، دون إعادة عنوان استجابة OAuth في التشخيص.

display=popup وgis_transform وresponse_type=token مع origin ومسارات oidclink/oauth/oidc
قرائن تحذير لا أدلة فشل. استجابة code أو access_token ذات جسم فارغ بعد ثلاث ثوانٍ تُحذّر
أيضاً؛ الفحص لا يقرأ نص الجسم إلى المضيف ولا يعمل أثناء handoff. لا يستنتج اكتمال الربط
من الإغلاق أو الصفحة البيضاء. قيود المزوّدين على المتصفحات المضمّنة ما زالت مستقلة.

حارس test:preview-popups يستخدم وحدة الإنتاج وElectron الفعلي وأصلين محليين وSession
تجربة معزولة. يقيس opener ورسالة ذات origin/source صحيحين وclose وبقاء الأب، وnoopener
وCOOP والحجب وحدّ العدد والتسليم وإذن معلق لابن من الأصل نفسه. الكوكي المقيس same-site
على منفذين مختلفين؛ لا يثبت third-party cookies. حارس الواجهة preview-controls-ui يقيس
الشريط وزر الطفل وأخطاء Console/CSP عبر واجهة الإنتاج وحد preload مضبوط.

لم تختبر هذه الحراس حسابات TikTok/Google أو Instagram/Meta أو Apple، ولا تدّعي نجاحها.
كاشف تدخل المستخدم لم يتغير: عدّاده يرتفع بأحداث الإدخال الملتزم، لا بتغيرات DOM.


### تنقّل OAuth الطويل: تعليق Facebook (2026-09-21)

أثبت رصد نسخة القبول أن طلب تنقّل HTTPS صادر من صفحة Facebook بطول 3111 محرفاً
وصل إلى will-navigate ثم صار defaultPrevented=true بسبب حد isHttpUrl البالغ 2048.
الدليل المنقّى: dist/popup-accept/facebook-diagnostic-1789942506199.jsonl، حدث
2026-09-20T22:15:57.714Z؛ لا يحوي عنوان العودة أو بيانات الدخول. رسالة تعذّر
CredentialManager ظهرت قبل محاولة الإرسال؛ لم تُعتمد سبباً لهذه العلّة.

يفصل isWebNavigationUrl صلاحية مخطط HTTP/HTTPS عن سقف إدخال الأداة isHttpUrl.
التنقّل الصادر من الصفحة، وتحويل HTTP، وإنشاء الابن، وعرض الأصل، وقراءة وجهة
الفعل تستخدم فحص المخطط؛ open/navigate ومدخلات الربط الخارجي تبقي حد 2048.
قراءة وجهة الفعل الطويلة لا ترجع إلى أصل الصفحة الأم، كي لا تعفي أصلاً جديداً
من بوابة الثقة. لم تتغير الكوكيز أو الجلسة أو sandbox أو أذونات المواقع.

حارس preview-popups أصبح 76 فحصاً حياً: يختبر عودة طويلة يصنعها renderer، وتحويل
302 إلى وجهة مختلفة، وابناً بعنوان طويل، وأصل إذن الفعل، وبقاء منع file/data.
زُرع حد 2048 مجدداً في أربعة مواضع مستقلة فسقط الحارس في كل منها ثم استُعيدت
بصمة المصدر؛ التقرير dist/facebook-navigation-fix/REPORT.md. هذه أدلة مسار محلي
حقيقي تحت Electron، وليست قبولاً لتسجيل حساب Instagram؛ قبول الحساب ينتظر
تجربة المالك في الحزمة المصححة. إغلاق نافذة Google ثبت بشهادة المالك، لكن
TikTok منع الدخول بسبب تجاوز المحاولات؛ لا تعاود تجربته ضمن إصلاح Facebook.

### منع تكرار الكتابة في الحقول الغنية — 2026-09-22

- ثبت في Electron أن مسار `browser_type` القديم يصدر `input` مرتين: واحداً أصلياً من
  `execCommand` ثم آخر مصطنعاً. هذا خلل قابل لإعادة الإنتاج؛ لم تُختبر صفحة واتساب نفسها.
- `browser_type` و`browser_fill_form` يستعملان استبدالاً واحداً للـ`contenteditable`،
  والمسح يأخذ أمر `delete`. لا حدث `beforeinput/input` مصطنع إضافي ولا fallback إلى
  `textContent` إذا رفض أمر التحرير. إعادة القيمة نفسها لا تضيف حدثاً أو نصاً.
- الحقول العادية تبقى على native value setter مع أحداثها القائمة. مسارات نقل الأسرار
  لم تتغير في هذه الدفعة. لا تتغيّر الأذونات أو الحفظ أو إرسال النماذج.
- `test:preview-member-live` يتحقق من النص وعدد الأحداث وأصالتها، وإعادة الكتابة والمسح
  والتعبئة ورفض التحرير دون فرض DOM بديل. نجاحه ليس قبولاً لواتساب أو لكل محرّر خارجي.

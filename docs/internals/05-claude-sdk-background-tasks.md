...
#### ملكية طلبات SDK عبر الأدوار — OBS-213 (2026-09-20، إصلاح محلي)

- عطل مثبت بمسبار يستدعي منطق الإنتاج: انتهاء دور آخر سحب بطاقة إذن بلا إرسال قرار للمحرك؛ بقي وعد canUseTool معلقاً. ينطبق السباق أيضاً على الأسئلة.
- main يولد ownerKey بالشكل sdk:<token> لكل permission_request وquestion_request؛ لا يُؤخذ من حمولة المحرك. result يحرر المؤلف ويبقي طلبات Query التي قد تحمل عملاً خلفياً.
- بعد حسم الإلغاء يبث agent حدث sdk_control_request_closed بمعرف الطلب ونوع permission أو question؛ main يقصر حقوله ويضيف المالك. main وحده يصدر sdk_control_owner_closed عند done أو فشل البدء، فيسحب بطاقات صاحبه فقط قبل حارس كتلة الدردشة. لا قناة IPC جديدة ولا قبول تلقائي.
- closeRequest وcloseOwner يراعيان الحالي والطابور وepoch الرد المعلق؛ قرار الجوال يسحب معرفه ومالكه المحددين. مسار saved_tasks مستقل كما كان.
- اجتازت الاختبارات المركزة (الملكية 27، الأذونات 56/56) وأربع عضّات مستقلة: حذف ملكية الإذن، حذف ملكية السؤال، تعطيل إشعار abort، وتعطيل تنظيف done؛ استُعيد المصدر ببصمته بعد كل واحدة. آخر طقم 147/148، وفشله الوحيد صيغة حالة OBS-213 في التوثيق؛ صُححت الحالة دون تخفيف الحارس. الأدلة المحلية في dist/claude-permission-review. هذه نتائج مصدر ومحركات بديلة للاختبار، لا إثبات زوال البلاغ على حزمة المالك المثبتة ولا قبول بشري جديد.
- إعادة الاتصال مسألة منفصلة: ثبت تقدم لاحق في سجلات المشروع، ولم يُحفظ سبب api_retry في الأدلة المتاحة. لم تُغيّر الشبكة أو المصادقة أو النسخة المثبتة.

- خاتمة التحقق المحلي: أُعيد test:observations بعد التصحيح فاجتاز 2154 فحصاً؛ بقي سجل الطقم الأصلي exit=1 كما حدث. نجح بناء Community إلى dist/claude-permission-review/package، وطابقت ملفات الإصلاح الستة داخل app.asar المصدر بايتياً. الدليل في package-audit.json. الحزمة لم تُطلق أو تُثبت؛ لا قبول بشري أو ادعاء حل سبب إعادة الاتصال.


- تجهيز الاستئناف (2026-09-20): تجربة المصدر live_94b64c5d84117f37f97f7af2 جاهزة بلا مصادقة أو طلب مدفوع. أعيد الحارسان 27 و56/56 بنجاح؛ لم يعد الطقم الكامل. المعايير والميزانية المعتمدة في متابعة المالك والعزل وحدوده وموارد term_3/term_4 في [نقطة الاستئناف §21](82-pro-saved-tasks-v1.md). إعداد claudeMdExcludes في المنزل التجريبي وحده لا يغير الإنتاج. لا قبول حزمة أو إغلاق للبلاغ من هذا التجهيز.

- المحاولة الحقيقية الأولى (2026-09-20): أثبتت Agent خلفية وsdk_agent_state started، ثم توقفت بخطأ سائق كان ينتظر sdk_task_started حصراً. ثبت stopped وإغلاق sdk:1، ولم يختبر التداخل. آخر كلفة تراكمية $0.3894414 وفيها Haiku مساعد غير محسوم السبب. صُحح السائق محلياً بلا إعادة مدفوعة؛ [تفاصيل الاستئناف §22](82-pro-saved-tasks-v1.md).

- المحاولة التصحيحية المعتمدة (2026-09-20): ثبت حياً بقاء بطاقة sdk:2 عبر result لدور sdk:3، ثم وصول السماح والرفض إلى الوكيل من tool_result الحقيقي وأثر الملفات. أُغلق المالكان والطابور فارغ. eventTrace أثبت إسقاط نتيجة تكلفة متأخرة stale_token؛ المرصود $0.588292 جزئي. اختبار الإلغاء مجهز ولم يُشغل؛ الحزمة والقبول البشري باقيان. [النتيجة والحدود والاستئناف §23](82-pro-saved-tasks-v1.md).

- نجح اختبار إلغاء طلب حقيقي معلّق (2026-09-20): stopSdkTask للمهمة المطابقة أغلق الطلب نفسه ثم أوقف المهمة وأغلق sdk:4، بلا كتابة الملف. الدليل والتكلفة الجزئية $0.8197003 في [§24](82-pro-saved-tasks-v1.md). فحص الحزمة والقبول البشري باقيان.

- أُطلقت الحزمة المحلية نفسها بعزل home/profile/downloads مقيس عبر inspector قبل main، والأمان باقٍ؛ تنتظر نقرة رفض بشرية على محاكاة معلنة بلا محرك. هذا لا يثبت Claude حقيقياً داخل الحزمة. [الحدود والدليل §25](82-pro-saved-tasks-v1.md).

- تصحيح تجهيز القبول: شاشة الإعداد عادت فوق البطاقة بعد فحص إقلاع متأخر؛ أصلح مشغّل المحاكاة وحده وقاس الوصول للزر. سُجلت نقرة رفض بشرية موثوقة واختفاء البطاقة، والتأكيد البصري للمالك منتظر. [التفاصيل §26](82-pro-saved-tasks-v1.md).

- وصلت لقطة المالك بعد الرفض: واجهة سطر ظاهرة دون شاشة الإعداد أو بطاقة معلقة. اكتمل فحص هذا الأثر البشري في المحاكاة المحلية، وأوقفت مهمتا التجربة وحدهما مع حفظ الأدلة. حدود إثبات المصدر والحزمة وإعادة الاتصال باقية؛ [نقطة الاستئناف §27](82-pro-saved-tasks-v1.md).

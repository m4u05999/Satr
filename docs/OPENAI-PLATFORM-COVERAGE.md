...
|---|---:|---:|---|---|---|
| `gpt-6-astra` | 1.05M / 128K (‏922K إدخال) | 10 / 1 / 50 | low→max | قدرات Sol نفسها | الرائد (‏2026-09-03). **الأغلى بمرتبة**: 4× إدخال Terra و3.3× إخراجه — يُطلب صراحةً ولا يكون افتراضاً |
| `gpt-5.6-cyber` | **400K** / 128K (‏272K إدخال) | **12.5 / 1.25 / 75** | غير معدودة رسمياً | فرع أمنيّ من أوزان Sol | **محجوب خلف Daybreak Red**: لا يوجد لحساب غير مفحوص. أبحاث الثغرات المصرَّح بها |
...
**حالة التحقق من `gpt-6-astra` (2026-09-05)**: الحقول أعلاه من صفحته الرسمية، والمعرّف
**مُثبَت بدليل فارق** لا بالتوثيق وحده — نداء حيّ على `/v1/responses` بمعرّف وهمي يردّ
`404 model_not_found`، بينما `gpt-6-astra` يردّ `429 credit_balance_exhausted` **تماماً
كـ`gpt-5.6-terra` المعروف**؛ أي أن OpenAI تتحقق من المعرّف قبل الرصيد، فالمعرّف صحيح
ومتاح للحساب. **وما لم يُثبَت**: استدلال ناجح فعليّ (شكل الرد وسلوك الجهد والبثّ) — حساب
المطوّر بلا رصيد، وهو القيد نفسه المسجَّل في `docs/GENERATION-PLAN.md` لمزوّد openai.

**حالة `gpt-5.6-cyber` (2026-09-05) — مختلفة جوهرياً**: المسبار نفسه يردّ عليه
`404 model_not_found` **تماماً كمعرّف وهمي** (`gpt-5.6-fake-xyz`)، بينما يردّ `429` على
`gpt-6-astra` و`gpt-5.6-terra`. وهذا **ليس خطأً في المعرّف** بل حجبٌ على مستوى الحساب:
النموذج يُزوَّد فقط بعد فحص **Daybreak Red**، ومصدر خارجي يصرّح بذلك حرفياً — «لا يوجد
معرّف تضعه في طلب API، بل تتقدّم وتُفحَص وعندها فقط يوجد النموذج بالنسبة لك».
⇒ **لا شيء من قدراته متحقَّق منه هنا**: مستويات الجهد وُرِثت عن Sol بحكم النسب المعلن
(أوزان واحدة) وتبقى **غير مثبتة**، والوسم في قائمة «سطر» يقول ذلك صراحةً كي لا يُقرأ
الحجبُ عطلاً في التطبيق.
